...
public class Appointment {
    ...
    public static ObservableList apptList() {
        ArrayList<LocalTime> t = new ArrayList<>();
        ArrayList<LocalTime> tET = new ArrayList<>();

        t.add(LocalTime.of(8, 0));
        for (int i = 0; i < 55; i++) {
            t.add(t.get(i).plusMinutes(15));
        }
        for (LocalTime lt :
                t) {
            tET.add(TimeConversions.timeConversion(LocalDateTime.from(lt.atDate(LocalDate.now()))));
        }
        return FXCollections.observableArrayList(tET);
    }
