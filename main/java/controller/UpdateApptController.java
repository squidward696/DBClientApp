    ...
    public void updateHandler(ActionEvent actionEvent) throws SQLException, IOException {
        ...
        for (Appointment x : UserController.apptData
            ...
            if (a.getStart().isBefore(x.getEnd()) && a.getEnd().isAfter(x.getStart()) && (a.getCustomerID() == x.getCustomerID() && a.getAppointmentID() != x.getAppointmentID())) {
                ...
                alert.setHeaderText("Appointment overlap for selected customer");
                alert.showAndWait();
                return;
            }
        }
        ...
        if (a.getEnd().toLocalTime().isAfter(TimeConversions.convertESTToSD(LocalDateTime.from(LocalTime.of(22, 0).atDate(a.getEnd().toLocalDate()))).toLocalTime()) ||
                a.getEnd().toLocalTime().isBefore(TimeConversions.convertESTToSD(LocalDateTime.from(LocalTime.of(8, 0).atDate(a.getEnd().toLocalDate()))).toLocalTime())) {
        ...
        // convert time to EST and then to UTC before updating in the database
//        a.setStart(TimeConversions.convertSDToUTC(a.getStart()));
//        a.setEnd(TimeConversions.convertSDToUTC(a.getEnd()));
