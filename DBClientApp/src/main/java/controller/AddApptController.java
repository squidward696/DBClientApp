...
public class AddApptController implements Initializable {
    ...
    public void createHandler(ActionEvent actionEvent) throws IOException, SQLException {
        ...
        Appointment appointment = new Appointment(apptID, title, desc, loc, type, start, end, custID, userID, contact);

        // check for appointment overlap
        for (Appointment x : UserController.apptData
        ) {
            if (appointment.getStart().isBefore(x.getEnd()) && appointment.getEnd().isAfter(x.getStart()) && (x.getCustomerID() == appointment.getCustomerID())) {
        ...
        if (end.toLocalTime().isAfter(TimeConversions.timeConversion(LocalDateTime.from(LocalTime.parse("22:00:00").atDate(end.toLocalDate())))) &&
                end.toLocalTime().isBefore(TimeConversions.timeConversion(LocalDateTime.from(LocalTime.parse("08:00:00").atDate(end.toLocalDate()))))) {
        ...
        // convert times to UTC before inserting into the database
        appointment.setStart(TimeConversions.convertToUTC(appointment.getStart()));
        appointment.setEnd(TimeConversions.convertToUTC(appointment.getEnd()));

        Query.appointmentsDAO.insert(appointment);
